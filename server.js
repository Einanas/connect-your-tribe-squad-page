...
// const squadData = await fetchJson(baseURL + 'squad')

// array klaar zetten voor waarin globale berichten voor message board op komt te staan
// const messages = []
...
// Old code cuz i did someting wrong and i dont know what
...
app.post('/', function (request, response) {
  // Voeg het nieuwe bericht toe aan de messages array
  message.push(request.body.bericht)
...
app.post('/detail/:id/like-or-unlike', function (request, response) {
  ...
  fetchJson('https://fdnd.directus.app/items/person/' + request.params.id).then((apiResponse) => {
    ...
    try {
      apiResponse.data.custom = JSON.parsel(apiResponse.data.custom)
    } catch (e) {
